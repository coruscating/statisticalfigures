    ...
    for filename in find_ext("../protocols/"+directory,"txt"):
        ...
        for i in line_iter:
            if "Score Score" in content[i]:
                ...
                jumptypes=[0,0,0,0,0] # counter for each jump type
                elenp=1
                j=0
                while True:
                    line=content[i+j].replace(" ",",").split(",")
                    elen=line[0]
                    if str(elen) != str(elenp): # BV has decimal digits, element #s do not
                        break
                    elenp+=1
                    j+=1
                ...
                fo2.write(content[i-1].split(' ')[0]+","+",".join(str(jt) for jt in jumptypes)+",")
                i+=1
                for j in range(5):
